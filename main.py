...
def draw_heatmap(df_new_heatmap, title):
    ...
    heatmap = sns.heatmap(df_new_heatmap, annot=True, fmt=".4f", cmap="RdYlGn_r",
...
def main():
    ...
    plt.title('EDAS annual rankings')
    plt.tight_layout()
    ...
    # EDAS orders preferences in descending order
    met = 'edas'
    type = 1
    ...
    # EDAS has descending ranking from prefs
    rank = rank_preferences(final_S, reverse = True)
    # summary_corrs['DARIA-EDAS'] = rank
    summary_corrs['2017-2022'] = rank
    summary_corrs.to_csv('./results/summary.csv')

    results_final = pd.DataFrame(index = country_names)
    results_final['DARIA-EDAS pref'] = final_S
    results_final['DARIA-EDAS rank'] = rank
